...
impl<C: Curve, const H: bool> PedersenGadget<C, H> {
    pub fn commit(
        ...
        // NOTE: The original implementation pairs generators and uses
        // `joint_scalar_mul_be` for performance, but that method relies on
        // incomplete addition formulas and can become unsatisfiable for some
        // inputs/parameters. For VRBDecode we prefer robustness/completeness
        // over the ~2x constraint savings here.
        let mut res = C::Var::zero();
        if H {
            res += h.scalar_mul_le(r.iter())?;
        }
        for (gi, vi_bits) in g.iter().zip(v.iter()) {
            res += gi.scalar_mul_le(vi_bits.to_bits_le()?.iter())?;
        }
        Ok(res)
    }
}
