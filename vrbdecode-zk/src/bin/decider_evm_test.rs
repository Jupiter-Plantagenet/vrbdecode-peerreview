...
fn main() -> Result<(), Error> {
    ...
    println!("=== Testing SNARK Wrapper with Minimal Configuration ===");
    println!("K=64, N={}", n_steps);

    let f_circuit = StepFCircuit::<64>::new_default()?;
    let poseidon_config = poseidon_canonical_config::<Fr>();
    let mut rng = StdRng::seed_from_u64(123456789u64);

    println!("Step 1: Nova preprocessing...");
    let nova_preprocess_start = Instant::now();
    let nova_preprocess_params = PreprocessorParam::new(poseidon_config, f_circuit.clone());
    let nova_params = N::preprocess(&mut rng, &nova_preprocess_params)?;
    let nova_preprocess_time = nova_preprocess_start.elapsed();
    println!("✅ Nova preprocessing: {:.2}s", nova_preprocess_time.as_secs_f64());

    println!("Step 2: Groth16 preprocessing (this is the expensive part)...");
    let groth16_preprocess_start = Instant::now();
    let (decider_pp, decider_vp) =
        D::preprocess(&mut rng, (nova_params.clone(), f_circuit.state_len()))?;
    let groth16_preprocess_time = groth16_preprocess_start.elapsed();
    println!("✅ Groth16 preprocessing: {:.2}s", groth16_preprocess_time.as_secs_f64());

    println!("Step 3: Nova folding for {} steps...", n_steps);
    ...
    let policy_hash_field =
        protocol::policy_hash_field(64, top_k, top_p_q16, t_q16, max_tokens, HASH_FN_ID, EXP_APPROX_ID);
    ...
    println!("✅ Nova folding: {:.2}s", fold_time.as_secs_f64());

    println!("Step 4: Groth16 wrapping...");
    let wrap_start = Instant::now();
    
    // Extract the values we need for verify/calldata before moving nova
    let i = nova.i;
    ...
    let proof = D::prove(&mut rng, decider_pp, nova)?;
    let wrap_time = wrap_start.elapsed();
    println!("✅ Groth16 wrapping: {:.2}s", wrap_time.as_secs_f64());
    ...
    println!("✅ Verification: {:.4}s", verify_time.as_secs_f64());

    println!("Step 6: EVM calldata preparation...");
    let calldata_start = Instant::now();
    ...
    let calldata_time = calldata_start.elapsed();
    println!("✅ Calldata preparation: {:.4}s", calldata_time.as_secs_f64());
    ...
    if solc_available() {
        println!("Step 7: EVM verification...");
        ...
        println!("✅ EVM verification: {:.4}s", evm_time.as_secs_f64());
        println!("   - Gas used: {}", gas_used);
        println!("   - Result: {:?}", evm_ok);
    } else {
        println!("⚠️  solc not found; skipping EVM verification");
    }

    println!("\n=== SUMMARY ===");
    println!("Nova preprocessing: {:.2}s", nova_preprocess_time.as_secs_f64());
    println!("Groth16 preprocessing: {:.2}s", groth16_preprocess_time.as_secs_f64());
    println!("Nova folding ({} steps): {:.2}s", n_steps, fold_time.as_secs_f64());
    println!("Groth16 wrapping: {:.2}s", wrap_time.as_secs_f64());
    println!("Verification: {:.4}s", verify_time.as_secs_f64());
    println!("Proof size: {} bytes", calldata.len());
    if let Some(gas) = evm_gas_used {
        println!("EVM gas: {}", gas);
    }
    ...
    if json_only {
        let output = serde_json::json!({
            "k": 64,
            "n_steps": n_steps,
            "nova_preprocess_time_s": nova_preprocess_time.as_secs_f64(),
            "groth16_preprocess_time_s": groth16_preprocess_time.as_secs_f64(),
            "fold_time_s": fold_time.as_secs_f64(),
            "wrap_time_s": wrap_time.as_secs_f64(),
